...
class SellModelCell: UITableViewCell,UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout {
    ...
    override func awakeFromNib() {
        ...
        modelCollectionViewHeight.constant = 80
        modelCollectionView.layoutIfNeeded()
    }
    
    func updateModelData(makeData:[Model]){
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        selectedModel?(updateData[indexPath.row].model ?? "N/A",updateData[indexPath.row].id ?? -1)
    }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.cellLabel.text = updateData[indexPath.row].model
        
        
        if AppUtility.isfromBuy{
            if AppUtility.isfromHome {
                if updateData[indexPath.row].id == AppUtility.selectedBuyFilterHome?.model{
                    modelCollectionView.selectItem(at: indexPath, animated: true, scrollPosition: .bottom)
                    AlreadyselectedModel?(updateData[indexPath.row].model ?? "N/A",updateData[indexPath.row].id ?? -1)
                    
                }
            }else{
                if updateData[indexPath.row].id == AppUtility.selectedBuyFilter?.model{
                    modelCollectionView.selectItem(at: indexPath, animated: true, scrollPosition: .bottom)
                    AlreadyselectedModel?(updateData[indexPath.row].model ?? "N/A",updateData[indexPath.row].id ?? -1)
            ...
            if AppUtility.isfromHome {
                if updateData[indexPath.row].id == AppUtility.selectedSellFilterHome?.model{
                    modelCollectionView.selectItem(at: indexPath, animated: true, scrollPosition: .bottom)
                    AlreadyselectedModel?(updateData[indexPath.row].model ?? "N/A",updateData[indexPath.row].id ?? -1)
                    
                }
            }else{
                if updateData[indexPath.row].id == AppUtility.selectedSellFilter?.model{
                    modelCollectionView.selectItem(at: indexPath, animated: true, scrollPosition: .bottom)
                    AlreadyselectedModel?(updateData[indexPath.row].model ?? "N/A",updateData[indexPath.row].id ?? -1)
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize{
        ...
        self.demoLabel.text = updateData[indexPath.row].model
