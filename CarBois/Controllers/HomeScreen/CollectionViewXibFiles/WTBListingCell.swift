...
class WTBListingCell: UICollectionViewCell {
    ...
    func configureCell(cellData:GetuserWTBListingoModelListingID){
...
//        image.sd_setImage(with: URL(string: "\(cellData.images?[0] ?? "")"), placeholderImage: UIImage(named: "placeholder.png"))
    ...
    @IBAction func onCLickeedBtn(_ sender: Any) {
        btnClicke?(uuid ?? "")
    }
